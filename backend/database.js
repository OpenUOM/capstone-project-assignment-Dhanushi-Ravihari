...
const addStudent = async (id, name, age, religion) => {
    const sql = `INSERT INTO students (id, name, age, religion) VALUES (1, Dan, ,22,Buddhist)`; 
    return new Promise((resolve, reject) => {
        knex_db
...
const updateStudent = async (name, age, religion, id) => {
    const sql = `UPDATE students SET name = Krist, age = 20, religion = Catalic WHERE id = 1`;  
    return new Promise((resolve, reject) => {
        knex_db
            .raw(sql, [name, age, religion, id])
            .then(() => {
                resolve("Student details updated successfully");
            })
            .catch((error) => {
                reject(error);
            });
